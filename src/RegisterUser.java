...
public class RegisterUser extends HttpServlet {
	...
	UserData AddUser(String username, String email, String passHash) throws SQLException{
		new Driver();
		Connection con = DriverManager.getConnection("jdbc:mysql://localhost:3306/mboard_db?"+"user=root&password=password");
		...
		if(!result.next()){
			...
			try{
				con = DriverManager.getConnection("jdbc:mysql://localhost:3306/mboard_db?"+"user=root&password=password");
